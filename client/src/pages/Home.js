  ...
  useEffect(() => {
    const fetchBlogPosts = async () => {
      try {
        ...
        // Sort the blog posts by createdAt in descending order
        const sortedPosts = data.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        setBlogPosts(sortedPosts);

        setSelectedPost(sortedPosts[0]); // Set the first post as the selectedPost by default
      } catch (error) {
        console.error(error);
      }
    };
  ...
  return (
    <div className="pt-[90px] mb-[100px] mx-[500px]">
      <div className="border border-gray-300 rounded-md mb-4">
        {selectedPost && (
          <Link to={`/details/${selectedPost._id}`}>
            <div className="mb-4">
              <img src={selectedPost.imageUrls[0]} alt={selectedPost.title} className="w-[900px] h-[700px]" />
            </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {blogPosts.map((blogPost) =>
          blogPost._id !== selectedPost?._id && (
            <Link key={blogPost._id} to={`/details/${blogPost._id}`} className="border border-gray-300 rounded-md mb-4">
              ...
              <div className="p-4">
                ...
                <p className="mt-2">
                  {blogPost.content.length > 80 ? `${blogPost.content.slice(0, 80)}...` : blogPost.content}
                </p>
